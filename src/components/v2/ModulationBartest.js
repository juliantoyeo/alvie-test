...
const ModulationBar = ({ from, initialMin, initialMax, data, width, onHourChangeEnd }) => {
    ...
    const test = (gestureState, fv) => {
        console.log(":::", gestureState.vx, fv)
        const xpos = gestureState.x0 + gestureState.dx
        const offset = (Dimensions.get('window').width - width) / 2
        let fv2 = (xpos - offset) / width * NUM_ITEMS
    ...
    const onMove = (evt, gestureState) => {
        ...
        // be careful with the problem of stale closure with react Hook
        let { min, max } =  selected

        // setSelected( ({min, max}) => {
        //     console.log("===",min,max, posInsideBar, gestureState.vx)
        //     return (selected)
        // })
        // console.log('---', posInsideBar)
        //console.log(min,max)
