...
export const HygoCardTransparentButton = ({ title, subtitle, text, buttonText, content = null, onPress }) => {

	return (
		<View style={[stylesTransparent.container, { marginBottom: 50 }]}>
			<View style={[stylesTransparent.left, { backgroundColor: COLORS.CYAN }]}></View>
			<View style={stylesTransparent.right}>
				<Text style={[stylesTransparent.cardTitle, { textAlign: 'center' }]}>{title}</Text>
				<View style={{alignItems: 'center', paddingTop: 10}}>{content}</View>
				<TouchableOpacity style={[stylesTransparent.bigButton, { backgroundColor: COLORS.CYAN }]} onPress={onPress}>
					<Text style={[stylesTransparent.buttonText, { textAlign: 'center' }]}>{buttonText}</Text>
				</TouchableOpacity>
