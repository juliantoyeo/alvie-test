	...
	return (
		<View style={styles.metrics}>
			<Text style={styles.metricsText}>
				{i18n.t(
					`meteo_overlay.precipitation_${getRFromProduct()}`,
					{ value: currentHourMetrics.prevprecipitation ? Math.round(currentHourMetrics.prevprecipitation + currentHourMetrics[getRFromProduct()]):'' })}
			</Text>
			<Text style={styles.metricsText}>
				{currentHourMetrics.t3 ? i18n.t(`realtime.gel_${currentHourMetrics.t3 <= -2 ? 'risky' : 'none'}`) : ''}
			</Text>
			<Text style={styles.metricsText}>
				{i18n.t(`meteo_overlay.delta_temp`, { value: currentHourMetrics.deltatemp ? Math.round(currentHourMetrics.deltatemp) : '' })}
			</Text>
		</View>
	)
}
