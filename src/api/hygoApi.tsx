...
export const hygoApi = axios.create({
    baseURL: __DEV__ ? 'http://ec2-54-216-119-26.eu-west-1.compute.amazonaws.com:3000' : 'https://hygo-api.alvie.fr', //'http://192.168.1.10:3000',//
    timeout: 300000,
    headers: {
...
export const getModulationValue_v4 = async (data): Promise<Array<number>> => {
    try {
        const response = await hygoApi.post('/app/modulation/v4', { ...data })
        return response.data
    } catch (error) {
        return []
    }
...
export const logError = async (errorMessage: string) => {
    try {
        const response = await hygoApi.post('/app/logError', {errorMessage})
        return (response.data)
    } catch(error) {
        console.log(error)
        return ({error})
